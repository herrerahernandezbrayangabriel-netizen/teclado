...
def normalizar_tecla(k: str) -> str:
    ...
    if k == "space":
        return "s"
    return k.replace("d", "f").replace("g", "", "space")

def mostrar_nota_visual(pagina, teclado, label, piano_nota, texto_mostrar, recursos, teclado_base):
    img_url = recursos.get(piano_nota, {Do, Re, Mi, Fa, Sol, La, Si, Do2,}).get("img","wav")
    if not img_url:
        return
    teclado.src = img_url
